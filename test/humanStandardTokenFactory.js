...
contract('HumanStandardTokenFactory', function (accounts) {
  it('Verify a Human Standard Token once deployed using both verification functions.', async () => {
    ...
    const newTokenAddr = await factory.createHumanStandardToken.call(
      100000,
      'Simon Bucks',
      2,
      'SBX',
    ...
    await factory.createHumanStandardToken(100000, 'Simon Bucks', 2, 'SBX', {
      from: accounts[0]
    })
    const res = await factory.verifyHumanStandardToken.call(newTokenAddr, {
      from: accounts[0]
    })
